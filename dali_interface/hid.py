# pyusb is not typed, so pyright cannot resolve any types. We're using strict typing
# mode globally, which conflicts with that. For that reason we're locally disabling the
# warnings about unknown types.
...
class DaliUsb(DaliInterface):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        devices = usb.core.find(find_all=True, idVendor=vendor, idProduct=product)  # pyright: ignore
        ...
        while devices[i]:
            ...
            interface = cfg[(0, 0)]  # pyright: ignore
            ...
            try:
                while True:
                    self.ep_read.read(getattr(self.ep_read, "wMaxPacketSize"), timeout=10)
                    logger.info("DALI interface - disregard pending messages")
            except Exception:
                pass
            super().__init__()
            # set the reply timeout to ample amount - timeout event is generated by interface
    ...
    def power(self, power: bool = False) -> None:
        ...
        if bytes_written != 64:
            raise Exception(f"written {bytes_written} bytes, expected 64.")

    def _transmit_locked(self, frame: DaliFrame, is_query: bool = False) -> None:
        """Transmit a DALI frame via USB interface.
        NOTE that the device will check for a backward frame after every transfer.
        So, if no backward frame is received there will be a timeout event in the
        receive queue.
        """
    ...
    def read_frame(self) -> DaliFrame | None:  # pylint: disable=too-many-branches
        """Read frame or event from USB DALI interface-"""
        logger.debug("enter read frame")
        try:
            assert self.ep_read
            logger.debug("read usb")
            usb_data = self.ep_read.read(getattr(self.ep_read, "wMaxPacketSize"), timeout=200)
            if usb_data:
